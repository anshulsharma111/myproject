from tkinter import *
import mysql.connector, datetime, math, os, random
...
class Application:
    def __init__(self, master):
        ...
        self.right = Frame(master, width=900, height=850, bg='lightblue')
        self.right.pack(side=RIGHT)

        # components
        self.heading = Label(self.left, bg='white', text='Clothing Collections', font=('arial 40 bold'))
    ...
    def ajax(self):
        ...
        for r in self.result:
            self.get_id = self.result[0]
            self.get_name = self.result[1]
            self.get_price =self.result[4]
            self.get_stock =self.result[2]
...
##                self.add_to_cart_btn.destroy()
    ...
    def generate_bill(self):
        ...
        f.write("\n\n\t\t\tChange: Rs. " + str(str(self.to_give)))
        ...
        c.execute(initial, (product_id[self.x], ))
        result=c.fetchone()
        for i in products_list:
            for r in result:
                self.old_stock = result[2]
            ...
            sql2 = "INSERT INTO transactions (product_name, quantity, amount, date)VALUES(%s,%s,%s,%s)"
...
class UpdateProducts:
    ...
    def search(self):
        ...
        result=c.fetchone()
        for r in result:
            self.n1 = result[1]  # name
            self.n2 = result[2]  # stock
            self.n3 = result[3]  # cp
            self.n4 = result[4]  # sp
            self.n5 = result[5]  # totalcp
            self.n6 = result[6]  # totalsp
            self.n7 = result[7]  # assumed_profit
            self.n8 = result[8]  # vendor
            self.n9 = result[9]  # vendor_phone
    ...
    def update(self):
        ...
        query = "UPDATE inventory SET name=%s, stock=%s, cp=%s, sp=%s, totalcp=%s, totalsp=%s, vendor=%s, vendor_phoneno=%s WHERE id=%s"
...
class Database:
    ...
    def get_items(self):
        ...
        if self.name == ' ' or self.stock == ' ' or self.cp == ' ' or self.sp == ' ':
            ...
            sql = "INSERT INTO inventory (id,name, stock, cp, sp, totalcp, totalsp, assumed_profit, vendor, vendor_phoneno)VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            parameters = (self.id,self.name, self.stock, self.cp, self.sp, self.totalcp, self.totalsp, self.assumed_profit, self.vendor, self.vendor_phone)
...
class Login:
    def __init__(self, master):
        ...
        self.Label1.place(relx=0.23, rely=0.38, height=50, width=600)
        self.Label1.configure(background="#0066ff", borderwidth="0", disabledforeground="#a3a3a3", font=font11, foreground="#ffffff", text='''Clothing Stock Management ''', width=154)
...
root = Tk()
Login(root)
root.title("Clothing Collections")
